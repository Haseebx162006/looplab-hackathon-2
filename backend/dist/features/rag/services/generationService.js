...
function validateBaseUrl(provider, url) {
    if (provider === 'custom') {
        if (!url || !url.startsWith('https://'))
            throw new Error('Custom providers require a valid HTTPS base URL');
        return url;
    }
...
async function generate({ system, user, generationConfig }) {
    ...
    try {
        if (['openai', 'groq', 'openrouter', 'custom'].includes(provider)) {
            ...
            if (!resp.ok)
                throw new Error(`Anthropic Error: ${await resp.text()}`);
            ...
            if (!resp.ok)
                throw new Error(`Gemini Error: ${await resp.text()}`);
            const data = await resp.json();
