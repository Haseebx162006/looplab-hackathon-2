...
async function chunkCount(mentorId) {
    const [rows] = await sequelize.query('SELECT count(*)::int AS n FROM knowledge_chunks WHERE mentor_id = :id', { replacements: { id: mentorId } });
    return rows?.[0]?.n ?? 0;
}
/**
...
async function getReadiness(mentorId) {
    ...
    const steps = [
        ...
        },
        {
            ...
            why: `Replies are grounded in your own material, never invented. ${chunks > 0
        ...
        },
        {
            ...
            why: `Drafts copy your tone rather than sounding like a chatbot. This happens on its own as you reply to mentees. ${styleExamples > 0 ? `${styleExamples} of your replies studied so far.` : ''}`,
