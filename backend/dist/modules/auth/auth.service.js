...
export class AuthService {
    static async signup(name, email, password) {
        ...
        if (existingUser) {
            ...
            await pool.query('UPDATE users SET name = $1, password_hash = $2 WHERE email = $3', [name.trim(), passwordHash, formattedEmail]);
        }
        else {
            // Create new user
            const passwordHash = await bcrypt.hash(password, 10);
            await pool.query('INSERT INTO users (name, email, password_hash, is_verified) VALUES ($1, $2, $3, false)', [name.trim(), formattedEmail, passwordHash]);
        }
        // Generate and save OTP
        const otp = generateOTP();
        const expiresAt = getOTPExpiry(5);
        await pool.query(`INSERT INTO otp_verifications (email, otp_code, expires_at, created_at, updated_at) 
       VALUES ($1, $2, $3, NOW(), NOW()) 
       ON CONFLICT (email) 
       DO UPDATE SET otp_code = EXCLUDED.otp_code, expires_at = EXCLUDED.expires_at, updated_at = NOW()`, [formattedEmail, otp, expiresAt]);
