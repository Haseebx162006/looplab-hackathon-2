...
export class BookingController {
    ...
    static async getAdminRequests(req, res, next) {
        try {
    ...
    static async respondToRequest(req, res, next) {
        try {
            ...
            const request = await BookingService.respondToRequest(adminId, id, decision, scheduledAt, durationMinutes ? parseInt(durationMinutes, 10) : 30, comment);
            res.status(200).json({
