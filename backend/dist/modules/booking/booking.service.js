...
export class BookingService {
    static async createRequest(userId, title, description) {
        if (!title || title.trim() === '') {
            throw { status: 400, message: 'Meeting title/topic is required.' };
        }
        const res = await pool.query(`INSERT INTO mentor_call_requests (user_id, title, description, status)
       VALUES ($1, $2, $3, 'pending')
       RETURNING *`, [userId, title, description || null]);
        return res.rows[0];
    }
    static async getUserRequests(userId) {
        const res = await pool.query(`SELECT * FROM mentor_call_requests 
       WHERE user_id = $1 
       ORDER BY created_at DESC`, [userId]);
        return res.rows;
    }
    static async getAdminRequests() {
        const res = await pool.query(`SELECT r.*, u.name as student_name, u.email as student_email
       FROM mentor_call_requests r
       JOIN users u ON r.user_id = u.id
       ORDER BY r.created_at DESC`);
        return res.rows;
    }
    static async respondToRequest(adminId, requestId, decision, scheduledAt, durationMinutes = 30, comment) {
        // 1. Get request and user email
        const reqCheck = await pool.query(`SELECT r.*, u.email as student_email, u.name as student_name
       FROM mentor_call_requests r
       JOIN users u ON r.user_id = u.id
       WHERE r.id = $1`, [requestId]);
        ...
        if (decision === 'approve') {
            ...
            // Update in DB
            const updateRes = await pool.query(`UPDATE mentor_call_requests 
         SET status = 'approved', meet_link = $1, scheduled_at = $2, comment = $3
         WHERE id = $4 RETURNING *`, [meetLink, startTime.toISOString(), comment || null, requestId]);
            // Send email
         ...
         WHERE id = $2 RETURNING *`, [comment || null, requestId]);
            // Send email
