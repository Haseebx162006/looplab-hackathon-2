...
export class ProgressService {
    static async getProgressSummary(userId) {
        ...
        const roadmapsRes = await pool.query(`SELECT r.id as roadmap_id, r.status, r.created_at, m.name as module_name
       FROM roadmaps r
       ...
       ORDER BY r.created_at DESC`, [userId]);
        ...
        for (const r of roadmaps) {
            const statsRes = await pool.query(`SELECT count(t.id) as total,
                count(t.id) FILTER (WHERE t.status = 'completed') as completed
         FROM roadmap_tasks t
         JOIN roadmap_sections s ON t.section_id = s.id
         WHERE s.roadmap_id = $1`, [r.roadmap_id]);
        ...
        const certsRes = await pool.query(`SELECT c.id as certificate_id, c.issued_at, m.name as module_name
       FROM certificates c
