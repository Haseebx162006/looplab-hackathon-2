...
export class TasksService {
    static async submitTask(userId, taskId, content, links) {
       ...
       WHERE t.id = $1`, [taskId]);
        ...
        // 2. Duplicate Check
        const pendingCheck = await pool.query("SELECT id FROM task_submissions WHERE task_id = $1 AND user_id = $2 AND status = 'pending_review'", [taskId, userId]);
    ...
    static async getTaskSubmissions(userId, taskId) {
        const taskCheck = await pool.query(`SELECT t.id, r.user_id
       FROM roadmap_tasks t
       JOIN roadmap_sections s ON t.section_id = s.id
       JOIN roadmaps r ON s.roadmap_id = r.id
       WHERE t.id = $1`, [taskId]);
        ...
        const submissionsRes = await pool.query('SELECT * FROM task_submissions WHERE task_id = $1 AND user_id = $2 ORDER BY created_at DESC', [taskId, userId]);
