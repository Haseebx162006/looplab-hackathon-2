...
async function seedAdmin() {
  ...
  try {
    ...
    if (adminCheck.rows.length === 0) {
      await pool.query(
        `INSERT INTO users (name, email, password_hash, is_verified, role, is_blocked)
         VALUES ($1, $2, $3, true, 'admin', false)`,
        ['System Admin', adminEmail, hashAdmin]
      );
      console.log(`✅ Seeded system admin user: ${adminEmail} (password: admin123)`);
    } else {
      await pool.query(
        "UPDATE users SET role = 'admin', is_verified = true, is_blocked = false WHERE email = $1",
        [adminEmail]
      );
    ...
    if (mentorCheck.rows.length === 0) {
      await pool.query(
        `INSERT INTO users (name, email, password_hash, is_verified, role, is_blocked)
         VALUES ($1, $2, $3, true, 'admin', false)`,
        ['Code Mentor', mentorEmail, hashMentor]
      );
