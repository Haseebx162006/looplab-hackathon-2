...
export class SkillSummariesService {
  static async generateSummary(userId: string, testId: string) {
    // 1. Fetch test
    ...
    const existingSummaryRes = await pool.query(
      'SELECT * FROM skill_summaries WHERE test_id = $1 AND user_id = $2',
      [testId, userId]
    );
    ...
    const insertRes = await pool.query(
      `INSERT INTO skill_summaries (user_id, module_id, test_id, strengths, weaknesses, skill_levels, missing_skills)
