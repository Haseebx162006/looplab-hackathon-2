...
import { GenerateTestInput, QuestionAnswer } from './tests.types.js';
...
export class TestsService {
  static async generateTest(userId: string, input: GenerateTestInput) {
    ...
    try {
      ...
      const testRes = await dbClient.query(
        `INSERT INTO tests (user_id, module_id, difficulty, status)
         VALUES ($1, $2, $3, 'in_progress')
         RETURNING *`,
        [userId, module_id, difficulty]
      );
      const test = testRes.rows[0];

      const questionPromises = questions.map((q) => {
        return dbClient.query(
          `INSERT INTO test_questions (test_id, question, options, correct_answer)
           VALUES ($1, $2, $3, $4)
           RETURNING id, question, options`,
          [test.id, q.question, q.options, q.correct_answer]
        );
      });
  ...
  static async submitTest(userId: string, testId: string, answers: QuestionAnswer[]) {
    const dbClient = await pool.connect();
    try {
      await dbClient.query('BEGIN');
      ...
      // 1. Fetch test
      const testRes = await dbClient.query(
        'SELECT * FROM tests WHERE id = $1 AND user_id = $2 FOR UPDATE',
        [testId, userId]
      );
      const test = testRes.rows[0];
      if (!test) {
      ...
      const questionsRes = await dbClient.query(
      ...
      for (const q of dbQuestions) {
        ...
        await dbClient.query(
          'UPDATE test_questions SET student_answer = $1 WHERE id = $2',
          [studentAnswer, q.id]
        );
      }
      ...
      // 4. Update test in DB
      await dbClient.query(
        "UPDATE tests SET status = 'completed', score = $1 WHERE id = $2",
        [score, testId]
      );
  ...
  static async getTestById(userId: string, testId: string) {
    ...
    const questionsRes = await pool.query(
      'SELECT id, question, options FROM test_questions WHERE test_id = $1',
      [testId]
    );
  ...
  static async getTestHistory(userId: string) {
