...
function getGmailClient() {
  ...
  try {
    const oauth2Client = new google.auth.OAuth2(
      GMAIL_CLIENT_ID,
      GMAIL_CLIENT_SECRET,
      'https://developers.google.com/oauthplayground' // standard redirect URI used for token generation/dev
    );
...
export async function sendOTPEmail(email: string, otp: string, expiresMinutes: number = 5): Promise<boolean> {
